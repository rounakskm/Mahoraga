...
**Delivery:** 🟡 **In progress.** Layer-1 *kernel* SHIPPED + runnable on real SPY (`strategy_template` + `eval` + `loop` + `scripts/run_autoresearch.py`, PRs #49/#50; runbook [`../../../runbooks/autoresearch-training.md`](../../../runbooks/autoresearch-training.md)). **Remaining Layer 1:** `experiments.iterations` Postgres + git registry (provenance) · vault-holdout. (Real RegimeDetector swap ✅ #52.) **Layers 2–3 (LLM mutator, agent fleet): not started.**
...
**Layer 1:** ✅ `strategy_template.py` · ✅ `eval.py` (+ metadata contract) · ✅ `loop.py` (mechanical) + `scripts/run_autoresearch.py` runner + live progress  ·  ✅ real Phase-1 MESO RegimeDetector (#52) · ⏳ `experiments.iterations` schema + writer · ⏳ git registry + provenance · ⏳ vault-holdout check
