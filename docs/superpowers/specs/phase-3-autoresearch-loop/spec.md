...
**Layer 1:** ✅ `strategy_template.py` · ✅ `eval.py` (+ metadata contract) · ✅ `loop.py` (mechanical) + `scripts/run_autoresearch.py` runner + live progress  ·  ✅ real Phase-1 MESO RegimeDetector (#52) · ⏳ `experiments.iterations` schema + writer · ✅ provenance (experiments.iterations + strategies.registry) · ✅ vault-holdout check
